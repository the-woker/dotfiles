...
Singleton {
    ...
    Process {
        id: scanner
        command: ["sh", "-c", "find ~/dotfiles/.config/quickshell/wallpaper/images/ -maxdepth 3 -type f \\( -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.png' -o -iname '*.webp' \\) 2>/dev/null | sort -u | head -200"]
        running: false
        stdout: SplitParser {
            onRead: data => {
